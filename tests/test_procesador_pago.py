"""
Archivo : test_procesador_pago.py
Descripción: Pruebas unitarias para el patrón Factory Method (ProcesadorPago).
             Verifica que cada Creator produce el Producto correcto,
             que el método pagar() es independiente del tipo, y el
...
from src.pago.procesador import (ProcesadorTarjeta, ProcesadorEfectivo,
                                  ProcesadorTransferencia, crear_procesador,
                                  ProcesadorPago)
...
def test_nuevo_tipo_pago_no_modifica_base():
    ...
    class PagoCrypto(Pago):
        def procesar(self, monto): print(f"[CRYPTO] {monto} BTC"); return True
        def tipo(self): return "crypto"

    class ProcesadorCrypto(ProcesadorPago):
        def crear_pago(self): return PagoCrypto()
