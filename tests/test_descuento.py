"""
Archivo : test_descuento.py
Descripción: Pruebas unitarias para el patrón Strategy (descuentos).
             Verifica el cálculo correcto de cada estrategia, el intercambio
             en tiempo de ejecución, el Principio Abierto/Cerrado al agregar
             nuevas estrategias, y todas las combinaciones via parametrize.
...
"""
import pytest
from src.descuento.estrategias import (SinDescuento, DescuentoPorVolumen,
                                        DescuentoCupon, DescuentoVIP,
                                        EstrategiaDescuento)
...
def test_nueva_estrategia_no_modifica_calculadora():
    class DescuentoLanzamiento(EstrategiaDescuento):
        def calcular(self, subtotal): return subtotal * 0.25
        def descripcion(self): return "25% lanzamiento"
...
@pytest.mark.parametrize("estrategia,subtotal,esperado", [
